...
Rather than thinking of the entire network as having one ledger, each trust-coin coin has its own uniquely tracked history.  Each time a coin changes hands is a generation.  Conflicting transaction messages can lead the history of a particular coin to fork, but this fork only applies to that coin, and only jeopardizes the value of that coin.  Peers actively participating in the network should be able to easily identify which fork is valid based on conventions for propagating messages and delaying confirmation of receiving coins.  All coin exchanges are double signed by both sender and receiver so that both parties attest that the transaction is valid.  Specifically, coin recipients should wait a stipulated amount of time before publishing a message acknowledging their receipt of the coin so that a conflicting simultaneous double spend will have time to be identified.  Both sender and recipient have an important responsibility in assuring that no double spends occur.
